...
function classifyError(err: unknown): ProxyErrorResponse {
  ...
  if (ax && status === 429) return { ok: false, error: {
  ...
  if (msg.includes("ENOTFOUND") || msg.includes("EAI_AGAIN") || msg.includes("getaddrinfo")) return { ok: false, error: {
  ...
  if (INPUT_ERROR_PHRASES.some(p => msg.includes(p))) return { ok: false, error: {
